...
def run_analysis(csv_path: str, sig_path: str,
    ...
    for defn in ["strict", "lenient"]:
        ...
        # Re-derive labels on FULL dataset (for accurate distribution reporting)
        df_full = derive_labels(raw, defn)
        n_full  = len(df_full)
        dist_full = df_full["severity_label"].value_counts()
        print(f"  Class distribution (full dataset, N={n_full:,}):")
        for tier in TIERS:
            cnt = dist_full.get(tier, 0)
            print(f"    {tier:<15} {cnt:>10,}  ({100*cnt/n_full:.1f}%)")

        # Feature engineering on full dataset
        df_full = engineer(df_full, sig)

        # Sample for CV only — distribution stored from full dataset above
        if sample and sample < len(df_full):
            df, _ = train_test_split(df_full, train_size=sample,
                                     stratify=df_full["severity_score"],
                                     random_state=rs)
            df = df.reset_index(drop=True)
            print(f"  Sampled to {len(df):,} for CV (stratified)")
        else:
            df = df_full
        ...
        defn_results = {
            "dist": dist_full,   # ← full-dataset distribution
            "n":    n_full,      # ← full N for percentage calculation
            "cv":   {}
        }
...
def write_excel(results: dict, out_path: str):
    ...
    # =========================================================================
    n_full = results["strict"]["n"]
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=7)
    c = ws.cell(row=row, column=1,
                value=f"A.  Class Distribution Under Each Definition  "
                      f"(full dataset, N = {n_full:,})")
